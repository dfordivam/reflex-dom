  ...
  describe "traverseDMapWithKeyWithAdjust" $ runSession $ do
    it "DMap patches render together" $ runWD $ do
      let
        ...
        patch1 = PatchDMap $ DMap.fromList
          [ Key_1 :=> (ComposeMaybe $ Just $ Identity 1)
          , Key_4 :=> (ComposeMaybe $ Just $ Identity "V4")
          , Key_5 :=> (ComposeMaybe $ Just $ Identity "V5a")
          ]
        patch2 = PatchDMap $ DMap.fromList
          [ Key_1 :=> (ComposeMaybe $ Just $ Identity 2)
          , Key_2 :=> (ComposeMaybe $ Just $ Identity 'b')
          , Key_3 :=> (ComposeMaybe $ Just $ Identity True)
          ]
        patch3 = PatchDMap $ DMap.fromList
          [ Key_1 :=> (ComposeMaybe $ Just $ Identity 3)
          , Key_4 :=> (ComposeMaybe Nothing)
          , Key_5 :=> (ComposeMaybe $ Just $ Identity "V5b")
          ]
        patch4 = PatchDMap $ DMap.fromList
          [ Key_1 :=> (ComposeMaybe $ Just $ Identity 4)
          , Key_2 :=> (ComposeMaybe $ Just $ Identity 'a')
          , Key_3 :=> (ComposeMaybe $ Just $ Identity False)
          , Key_5 :=> (ComposeMaybe Nothing)
          ]
        ...
        vals2 = mkVals (applyAlways (patch2 <> patch1) initialDMap)
        ...
        let
          ...
          elN n = do
            void $ elAttr "p" ("name" =: elemName) $ do
              traverseDMapWithKeyWithAdjust widget initialDMap patchEv
